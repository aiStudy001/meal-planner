...
홈 화면에서 4가지 미리 준비된 시나리오를 선택하여 즉시 시작할 수 있습니다:

1. **체중 감량 남성** - 고단백 저칼로리 식단
2. **체중 증가 여성** - 균형 잡힌 영양소로 건강한 체중 증가
3. **다중 알레르기** - 견과류, 해산물, 유제품 제외
4. **건강 제약 (당뇨+고혈압)** - 저염, 저당, 혈당 조절 식단

시나리오를 선택하면 자동으로 프로필이 설정되어 입력 페이지로 이동합니다.

### 📄 PDF 내보내기

완성된 식단을 PDF로 다운로드할 수 있습니다:

- **제목** 및 사용자 프로필 요약
- **일별 식단표** (식사 유형, 메뉴명, 칼로리, 비용)
- **일일 영양 요약** (칼로리, 단백질, 탄수화물, 지방)
- **주간 평균 영양** 정보
- **예산 요약**
...
전체 식단 데이터를 JSON 형식으로 다운로드:

- 프로그래밍 방식 처리 가능
- 데이터 백업 및 공유
- 외부 시스템 통합

파일명: `meal-plan-YYYY-MM-DD.json`

### 🛒 장보기 리스트

식단에 필요한 모든 재료를 자동으로 정리:

- **카테고리별 분류** (육류, 해산물, 채소, 유제품, 곡물, 조미료)
- **수량 자동 합산** (중복 재료 통합)
- **단위 정규화** (g, kg, ml, 개 등)
- **모달 UI** (사용하기 쉬운 인터페이스)
...
#### Core System (Phase 0)
- Multi-Agent 시스템 구현 (3 experts + 5 validators)
- Progressive Relaxation 재시도 전략
- SSE 실시간 스트리밍
- 4-step wizard 입력 폼
- 실시간 진행 상황 표시 (진행률, 에이전트 상태)
- 에러 배너 UI (실시간 에러 피드백)
- mise 기반 통합 개발 환경

#### 2025년 1월 업데이트
- ✅ Docker Compose 환경 (백엔드 + 프론트엔드)
- ✅ 4가지 테스트 시나리오 (체중 감량, 체중 증가, 다중 알레르기, 건강 제약)
- ✅ PDF 내보내기 (기본 버전: 식단표 + 영양 정보)
- ✅ JSON 내보내기 (전체 데이터)
- ✅ 장보기 리스트 (카테고리별 재료 정리)
- ✅ 시나리오 선택 드롭다운 (홈 화면)
